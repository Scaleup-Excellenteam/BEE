...
def test_duplicate_put_does_not_increase_frequency_or_replace_value() -> None:
    cache = QueryResultCache(capacity=2)
    cache.put("duplicate", [_result("First writer")])
    cache.put("duplicate", [_result("Second writer")])
    cache.put("other", [_result("Other")])

    cache.put("newest", [_result("Newest")])

    assert cache.get("duplicate") is None
    assert cache.get("other") == [_result("Other")]
    assert cache.get("newest") == [_result("Newest")]
    assert cache.info().evictions == 1
